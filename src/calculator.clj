...
(defn button-listener [frame name whattodo]
...
(defn add-button [frame label whattodo]
  (let [name (JButton. label)]
    (button-listener frame name whattodo)
    (.add frame name)
    name))
...
(defn apply-calc
  ...
  ([operation value frame]
    (.setText value (str (try (operation
  ...
  ([put-value clean-result]
    (let [frame (JFrame. "Calculator")
      ...
      (doto frame
        ...
        (add-button (if put-value "Result" "Quit")
          (fn [x] (if put-value
